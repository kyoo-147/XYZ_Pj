...
class Face_Recognizer:
    ...
    def draw_note(self, img_rd):
        cv2.putText(img_rd, "HE THONG DIEM DANH - AI-LAB", (20, 40), self.font, 1, (255, 255, 255), 1, cv2.LINE_AA)
        cv2.putText(img_rd, "SO KHUNG HINH:  " + str(self.frame_cnt), (20, 100), self.font, 0.8, (0, 255, 0), 1,
                    cv2.LINE_AA)
                        ...
                        for k in range(len(faces)):
                            ...
                            for i in range(len(self.face_features_known_list)):
                                if str(self.face_features_known_list[i][0]) != '0.0':
                                    ...
                                    # self.e_distance_accuracy = (linear_val + ((1.0-linear_val)*math.pow((linear_val-0.5)*2,0.2)))*100
                                    # logging.debug("Accuracy: with person %d, accuracy :%f%%",i+1, self.e_distance_accuracy)
                                    logging.debug("      with person %d, the e-distance: %f", i + 1, e_distance_tmp)
                                    self.current_frame_face_X_e_distance_list.append(e_distance_tmp)
                                    # accuracy_text = f"DO CHINH XAC: {self.e_distance_accuracy:.2f}%"
                                    # print(accuracy_text)
                                    # cv2.putText(img_rd, accuracy_text, (20,230), self.font, 0.8, (0,255,0), 1, cv2.LINE_AA)
                                    self.draw_note(img_rd)
