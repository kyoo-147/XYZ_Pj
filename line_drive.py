...
def draw_rectangle(img, lpos, rpos, offset=0):
    ...
    cv2.rectangle(img, (rpos - 5, 15 + offset),
                       (rpos + 5, 25 + offset),
                       (0, 255, 0), 2)   
    cv2.rectangle(img, ((rpos + lpos) // 2 - 5, 15 + offset), ((rpos + lpos) // 2 + 5, 25 + offset), (0, 255, 0), 2)

    cv2.rectangle(img, (Width//2 + 5, 15 + offset), (Width//2 + 15, 25 + offset), (0, 0, 255), 2)
...
def canny(image):
    gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY) # in black and white
    blur = cv2.GaussianBlur(gray, (5, 5), 0) # Blur processing
...
def process_image(frame):
    ...
    lane_image = np.copy(frame) #image copy
    ...
    cv2.imshow('check', frame) # Check images occasionally
    cv2.waitKey(1)
...
# I think I can touch it here

def draw_steer(image, steer_angle):
    global Width, Height, arrow_pic

    arrow_pic = cv2.imread('steering_wheel_image.jpg', cv2.IMREAD_COLOR)
    ...
    arrow_Width = int((arrow_Height * 462) / 728 + 10)
    ...
    image[int(Height - arrow_Height): int(Height), int(Width/2 - arrow_Width/2): int(Width/2 + arrow_Width/2)] = res
...
if __name__ == '__main__':
    ...
    while not rospy.is_shutdown():
        ...
        #print(slope[0])
        steer_angle = -(pos[1]+pos[0]-Width)/5 # Even if you just adjust this angle, the wheel will move.
